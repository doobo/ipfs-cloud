...
import com.github.doobo.model.Node;
...
public class UnionUtils {
	...
	public static boolean initLinux64Ipfs(){
		byte[] rs =FileUtils.readResourcesByte("linux64/go-ipfs.tar.gz");
		return initUnixIpfs(rs);
	}
	...
	public  static boolean initSub(IpfsConfig ipfsConfig) throws InterruptedException {
		...
		POOL.execute(()->{
			try {
				ScriptUtil.execCmdLine(UnionUtils.IPFS, null, new CollectingLog(ipfsConfig.getTopic()), Long.MAX_VALUE
					, "pubsub", "sub", ipfsConfig.getTopic(), "--encoding", "json"
					, UnionUtils.IPFS_CONF_ARRAY[0], UnionUtils.IPFS_CONF_ARRAY[1]);
			} catch (Exception e) {
