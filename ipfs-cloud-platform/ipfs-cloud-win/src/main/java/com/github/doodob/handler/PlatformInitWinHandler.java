...
public class PlatformInitWinHandler extends AbstractPlatformInitHandler {
	...
	public ResultTemplate<PlatformInitResponse> initIpfs(PlatformInitRequest request) {
		...
		try {
			...
			response.setExePath(new File(pwd + File.separator + "/go-ipfs/ipfs.exe").getCanonicalPath());
		}catch (Exception e){
		...
		if(!isInitIpfs(pwd)){
			...
			byte[] rs =FileUtils.readResourcesByte("win64/go-ipfs.zip");
			if(!initWinZipIpfs(rs, pwd)){
	 ...
	 * win环境初始化
	 */
	private static boolean initWinZipIpfs(byte[] rs, String pwd) {
		try (OutputStream out = new FileOutputStream(pwd+"/go-ipfs/ipfs.exe")){
			byte[] ipfs = CompressorUtils.queryFileInZip(rs, "go-ipfs/ipfs.exe");
			out.write(ipfs);
		} catch (Exception e){
			log.error("initWinZipIpfs fail:", e);
			return false;
		}
		return true;
	}
